...
class TrainConfig:
    ...
    loss_tolerance: float = 0.075  # if validation loss increases by this amount, stop training
...
class Trainer:
    ...
    def get_batch(self, split: str, train_data: torch.Tensor, val_data: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        ix = torch.randint(
            len(data) - self.model_config.context_size,
            (self.train_config.batch_size,)
        )
        # get sequences of context_size length
        x = torch.stack([
            data[i:i + self.model_config.context_size] for i in ix
        ])
    ...
    @torch.no_grad()
    def estimate_loss(self, train_data: torch.Tensor, val_data: torch.Tensor) -> Dict[str, float]:
